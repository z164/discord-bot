...
const register = async (message, body) => {
    ...
    if (message.author.id === message.guild.ownerID) {
        console.log(`${nickname.nicknameStyle} is a guild owner. Using bot's id to proceed`.warning)
    ...
    if (!guild) {
        ...
        console.log('Guild did not exist in database, so it was created'.warning)
    } else {
    ...
    User.create({
        ...
        if (err) {
            ...
            console.log(`${nickname.nicknameStyle} registered successfully with nickname ${dotaNickname.nicknameStyle}`.log)
            console.log(logUtilities.separator)
