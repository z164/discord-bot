...
const setUsersDotaNickname = async (message, body) => {
    console.log(`[${moment(new Date()).format('H:mm:ss').magenta}][${'Set'.title}]`)
    if (!message.member.hasPermission('ADMINISTRATOR')) {
        console.log('Set was invoked by non-administrator user'.error)
    ...
    await User.findOneAndUpdate({
        ...
        if (err) {
            ...
            if (res === null) {
                ...
                console.log(`Changed ${res.nickname.nicknameStyle}'s Dota 2 nickname to ${bodyStr.nicknameStyle}`.log)
                console.log(separator)
